...
_date_human_pat = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    ''' Find a date string which is in ISO8601 format '''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_human(text):
    ''' Find a human date string of format 25 Jan 2017 '''
    for match in _date_human_pat.finditer(text):
        yield('date', match)
